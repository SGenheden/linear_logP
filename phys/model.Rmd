...
```{r}
minisota<-read_excel("minnesota.xlsx")
minisota$HBonds<-minisota$HDonors+minisota$HAcceptors
minisota$Nonpolar<-minisota$Carbons/minisota$Heavies
f<-minisota[c("dGow")]
f$pred<-predict(mm, minisota[c("Weight","HBonds","Nonpolar")])
```

Some simple statistics of this model is MAD=`r round(mean(abs(f$dGow-f$pred)),2)` kcal/mol and $R^2$=`r round(cor(f$dGow,f$pred),2)`

and a correlation graph looks like this 

```{r}
library(ggplot2)
qplot(dGow, pred, data=f, xlab="Experimental [kcal/mol]", ylab="Predicted [kcal/mol]", xlim=c(-4,8), ylim=c(-4,8)) + theme(aspect.ratio=1)
```

This we can compare to the simulations method Elba, CG Martini and AA/CG Martini

```{r}
ols<-data.frame(exp=f$dGow,pred=f$pred,model="OLS")
